...
def check_servernet_ip(vm: vim.VirtualMachine, content: vim.ServiceInstanceContent):
    ...
    try:
        ...
        regex = "(?<=inet 10.43.)\d*(?=.5)"
        match = re.findall(regex, output)[0]
        teamname = vm.parent.name
        teamnumber = gradinglib.team_name_to_number(teamname)
        
        if(teamnumber == match):
            logger.info(f"SUCCESS! {vm.name} in {vm.parent.name} passed. UbuntuClient IP is correct.")
        else:
            logger.warn(f"FAILED! {vm.name} in {vm.parent.name} failed. UbuntuClient IP is incorrect.")
